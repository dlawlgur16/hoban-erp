...
export default async function NewDeliveryPage() {
  ...
  const [items, sites, orderLines, deliveryLines, locations] = await Promise.all([
    ...
    prisma.clientOrderLine.findMany({ where: { order: { clientId: client.id } } }),
  ...
  for (const [k, v] of stockMap.entries()) stockData.push({ key: k, ordered: v.ordered, delivered: v.delivered });

  return (
    <>
      <PageHeader
        title={`${client.name} 배송 등록`}
        description="사업소를 고른 후 품목별 박스 수량을 입력합니다."
      />
      <Section>
        <DeliveryForm
          mode="create"
